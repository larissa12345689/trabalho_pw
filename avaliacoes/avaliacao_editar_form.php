...
<body>
    <nav class="navbar-aval">
        ...
        <a href="listar_avaliacoes.php" class="link-voltar-nav">← Voltar</a>
    </nav>

    <div class="caixa-banho">
        <h2>Editar Avaliação ✏️</h2>
        <form action="processa_editar_avaliacao.php" method="POST">
            <div class="banho-grade">
                <div class="campo-grupo">
                    <label class="campo-label">Nome do Pet</label>
                    <input type="text" name="pet_nome" value="<?php echo htmlspecialchars($dados['pet_nome']); ?>" class="campo-input" required>
                </div>
                <div class="campo-grupo">
                    <label class="campo-label">Nota</label>
                    <select name="nota" class="campo-input" required>
                        <option value="5" <?php echo $dados['nota'] == 5 ? 'selected' : ''; ?>>⭐⭐⭐⭐⭐ Excelente</option>
                        <option value="4" <?php echo $dados['nota'] == 4 ? 'selected' : ''; ?>>⭐⭐⭐⭐ Muito Bom</option>
                        <option value="3" <?php echo $dados['nota'] == 3 ? 'selected' : ''; ?>>⭐⭐⭐ Bom</option>
                        <option value="2" <?php echo $dados['nota'] == 2 ? 'selected' : ''; ?>>⭐⭐ Regular</option>
                        <option value="1" <?php echo $dados['nota'] == 1 ? 'selected' : ''; ?>>⭐ Ruim</option>
                    </select>
                </div>
                <div class="campo-grupo" style="grid-column: span 2;">
                    <label class="campo-label">Seu Comentário</label>
                    <textarea name="comentario" class="campo-input" rows="4" required style="resize:vertical;"><?php echo htmlspecialchars($dados['comentario']); ?></textarea>
                </div>
                <input type="hidden" name="id" value="<?php echo $dados['id']; ?>">
            </div>
            <button type="submit" class="btn-banho">Salvar Alterações 💾</button>
        </form>
    </div>
